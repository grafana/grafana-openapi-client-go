// Code generated by go-swagger; DO NOT EDIT.

package licensing

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	"net/http"
	"time"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	cr "github.com/go-openapi/runtime/client"
	"github.com/go-openapi/strfmt"
)

// NewGetCustomPermissionsCSVParams creates a new GetCustomPermissionsCSVParams object,
// with the default timeout for this client.
//
// Default values are not hydrated, since defaults are normally applied by the API server side.
//
// To enforce default values in parameter, use SetDefaults or WithDefaults.
func NewGetCustomPermissionsCSVParams() *GetCustomPermissionsCSVParams {
	return &GetCustomPermissionsCSVParams{
		timeout: cr.DefaultTimeout,
	}
}

// NewGetCustomPermissionsCSVParamsWithTimeout creates a new GetCustomPermissionsCSVParams object
// with the ability to set a timeout on a request.
func NewGetCustomPermissionsCSVParamsWithTimeout(timeout time.Duration) *GetCustomPermissionsCSVParams {
	return &GetCustomPermissionsCSVParams{
		timeout: timeout,
	}
}

// NewGetCustomPermissionsCSVParamsWithContext creates a new GetCustomPermissionsCSVParams object
// with the ability to set a context for a request.
func NewGetCustomPermissionsCSVParamsWithContext(ctx context.Context) *GetCustomPermissionsCSVParams {
	return &GetCustomPermissionsCSVParams{
		Context: ctx,
	}
}

// NewGetCustomPermissionsCSVParamsWithHTTPClient creates a new GetCustomPermissionsCSVParams object
// with the ability to set a custom HTTPClient for a request.
func NewGetCustomPermissionsCSVParamsWithHTTPClient(client *http.Client) *GetCustomPermissionsCSVParams {
	return &GetCustomPermissionsCSVParams{
		HTTPClient: client,
	}
}

/*
GetCustomPermissionsCSVParams contains all the parameters to send to the API endpoint

	for the get custom permissions c s v operation.

	Typically these are written to a http.Request.
*/
type GetCustomPermissionsCSVParams struct {
	timeout    time.Duration
	Context    context.Context
	HTTPClient *http.Client
}

// WithDefaults hydrates default values in the get custom permissions c s v params (not the query body).
//
// All values with no default are reset to their zero value.
func (o *GetCustomPermissionsCSVParams) WithDefaults() *GetCustomPermissionsCSVParams {
	o.SetDefaults()
	return o
}

// SetDefaults hydrates default values in the get custom permissions c s v params (not the query body).
//
// All values with no default are reset to their zero value.
func (o *GetCustomPermissionsCSVParams) SetDefaults() {
	// no default values defined for this parameter
}

// WithTimeout adds the timeout to the get custom permissions c s v params
func (o *GetCustomPermissionsCSVParams) WithTimeout(timeout time.Duration) *GetCustomPermissionsCSVParams {
	o.SetTimeout(timeout)
	return o
}

// SetTimeout adds the timeout to the get custom permissions c s v params
func (o *GetCustomPermissionsCSVParams) SetTimeout(timeout time.Duration) {
	o.timeout = timeout
}

// WithContext adds the context to the get custom permissions c s v params
func (o *GetCustomPermissionsCSVParams) WithContext(ctx context.Context) *GetCustomPermissionsCSVParams {
	o.SetContext(ctx)
	return o
}

// SetContext adds the context to the get custom permissions c s v params
func (o *GetCustomPermissionsCSVParams) SetContext(ctx context.Context) {
	o.Context = ctx
}

// WithHTTPClient adds the HTTPClient to the get custom permissions c s v params
func (o *GetCustomPermissionsCSVParams) WithHTTPClient(client *http.Client) *GetCustomPermissionsCSVParams {
	o.SetHTTPClient(client)
	return o
}

// SetHTTPClient adds the HTTPClient to the get custom permissions c s v params
func (o *GetCustomPermissionsCSVParams) SetHTTPClient(client *http.Client) {
	o.HTTPClient = client
}

// WriteToRequest writes these params to a swagger request
func (o *GetCustomPermissionsCSVParams) WriteToRequest(r runtime.ClientRequest, reg strfmt.Registry) error {

	if err := r.SetTimeout(o.timeout); err != nil {
		return err
	}
	var res []error

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}
